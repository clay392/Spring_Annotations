package com.itheima.typefilter;
...
public class DistrictTypeFilter extends AbstractTypeHierarchyTraversingFilter {
    ...
    protected boolean matchClassName(String className) {
        try {
             ...
             */
            if (!isPotentPackageClass(className)){
                //不符合路径规则
                return false;
            }
            //判断当前区域和配置区域是否一致,不一致不能注册到Spring的Ioc容器中
     ...
     * @return
     */
    private boolean isPotentPackageClass(String className){
        //1、将类名转换成为资源路径,已匹配是否符合扫描条件
